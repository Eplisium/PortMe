...
class PortScanner:
    ...
    async def async_scan_host_ports(self, host: str, ports: List[int], protocols: List[str] = None) -> List[ScanResult]:
        ...
        # Enrich results (process info, WSL/Docker) similar to sync path
        try:
            port_proc_map: Dict[int, Dict[str, str]] = {}
            docker_map: Dict[int, Dict[str, str]] = {}
            wsl_map: Dict[int, Dict[str, str]] = {}
            ...
            if results:
                if port_proc_map:
                    for r in results:
                        self._enrich_with_process_info(r, port_proc_map)
                if docker_map or wsl_map:
                    for r in results:
                        self._enrich_with_envs(r, wsl_map, docker_map, wsl_distro)
        except Exception as e:
            logger.debug(f"Async enrichment failed: {e}")
        
        # Sort results by port number, then by protocol
        results.sort(key=lambda x: (x.port, x.protocol))
        
        # Keep parity with sync path: store results on the scanner instance
        self.results.extend(results)
        return results
...
"""
        ...
        html_content += f"""
        <div class="footer">
            <p>Report generated by Advanced Port Scanner | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} • Made with loveee &lt;3 :3</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    def export_results(self, filename: str, format: str = "json", gzip_json: Optional[bool] = None):
        ...
        try:
            if format.lower() == "json":
                ...
                # Determine gzip behavior: explicit True, or if filename ends with .gz/.json.gz
                use_gzip = False
                if gzip_json is True:
                    use_gzip = True
                elif gzip_json is None and (filename.endswith('.gz') or filename.endswith('.json.gz')):
                    use_gzip = True
                ...
                # Build Markdown table export
                lines = []
                ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                lines.append(f"# Scan Results\n")
                lines.append(f"_Generated: {ts}_\n")
                lines.append("")
                # Cute note requested by user
                lines.append("Made with loveee <3 :3\n")
                lines.append("")
                headers = [
                    "Host","Port","Proto","Status","Service","ServiceVersion","Conf",
                    "Banner","RespTime","PID","Process","LocalAddr","WSL","Docker"
                ]
                lines.append("| " + " | ".join(headers) + " |")
                lines.append("|" + "|".join(["---"]*len(headers)) + "|")
                for r in self.results:
                    banner = (r.banner or '').replace("|", "\|").replace("\n"," ")
                    service_version = getattr(r, 'service_version', '') or ''
                    confidence = getattr(r, 'confidence', 0.0) or 0.0
                    
                    docker = (r.docker_container or '')
                    row = [
                        r.host,
                        str(r.port),
                        r.protocol,
                        ...
                        str(r.pid or ''),
                        (r.process or '').replace("|","\|"),
                        (r.local_address or ''),
                        "yes" if r.is_wsl else "no",
                        docker
                    ]
                ...
                try:
                    ...
                    raise RuntimeError("Excel export requires openpyxl. Install with: pip install openpyxl") from e
                ...
                # Header note
                note = "Scan Results - Made with loveee <3 :3"
                ...
                headers = [
                    "Host","Port","Protocol","Status","Service","ServiceVersion","Confidence",
                    "Banner","ResponseTime","PID","Process","ProcessPath","LocalAddress","IsWSL",
                    "WSLDistro","WSLProcess","WSLPath","DockerContainer","DockerImage","DockerContainerID"
                ]
                ...
                # Data rows
                for row_idx, r in enumerate(self.results, start=3):
                    ws.cell(row=row_idx, column=1, value=r.host)
                    ws.cell(row=row_idx, column=2, value=r.port)
                    ws.cell(row=row_idx, column=3, value=r.protocol)
                    ws.cell(row=row_idx, column=4, value=r.status)
                    ws.cell(row=row_idx, column=5, value=r.service)
                    ws.cell(row=row_idx, column=6, value=getattr(r, 'service_version', '') or '')
                    ws.cell(row=row_idx, column=7, value=float(getattr(r, 'confidence', 0.0) or 0.0))
                    ws.cell(row=row_idx, column=8, value=(r.banner or ''))
                    ws.cell(row=row_idx, column=9, value=float(r.response_time or 0.0))
                    ws.cell(row=row_idx, column=10, value=(r.pid if r.pid is not None else ''))
                    ws.cell(row=row_idx, column=11, value=(r.process or ''))
                    ws.cell(row=row_idx, column=12, value=(r.process_path or ''))
                    ws.cell(row=row_idx, column=13, value=(r.local_address or ''))
                    ws.cell(row=row_idx, column=14, value=bool(r.is_wsl))
                    ws.cell(row=row_idx, column=15, value=(r.wsl_distro or ''))
                    ws.cell(row=row_idx, column=16, value=(r.wsl_process or ''))
                    ws.cell(row=row_idx, column=17, value=(r.wsl_path or ''))
                    ws.cell(row=row_idx, column=18, value=(r.docker_container or ''))
                    ws.cell(row=row_idx, column=19, value=(r.docker_image or ''))
                    ws.cell(row=row_idx, column=20, value=(r.docker_container_id or ''))

                # Freeze header rows and auto-width
                ws.freeze_panes = "A3"
                ...
                for row in ws.iter_rows(min_row=2, max_row=ws.max_row, values_only=True):
                    for idx, val in enumerate(row, start=1):
                        ...
                        if l > max_len.get(idx, 0):
                            max_len[idx] = min(l, 60)
                for idx, width in max_len.items():
...
Configuration Examples:
  ...
  %(prog)s --create-config config.yaml  # Create sample config
    ...
    parser.add_argument('-o', '--output',
                       help='Output file (supports .json, .json.gz, .csv, .html, .md, .xlsx)')
    parser.add_argument('--gzip', dest='gzip_output', action='store_true',
                       help='Compress JSON output with gzip (.json.gz). Implied when --output ends with .gz')
    ...
    try:
        ...
        if args.output:
            ...
            if out_lower.endswith('.html'):
                format_type = "html"
            elif out_lower.endswith('.json') or out_lower.endswith('.json.gz') or out_lower.endswith('.gz'):
                format_type = "json"
                ...
                format_type = "csv"
            use_gzip = bool(getattr(args, 'gzip_output', False)) or out_lower.endswith('.gz') or out_lower.endswith('.json.gz')
            scanner.export_results(args.output, format_type, gzip_json=use_gzip)
